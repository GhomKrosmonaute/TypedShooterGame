...
export default class CircularSaw extends Enemy {
  ...
  constructor(party: Party) {
    ...
    this.onDraw = function () {
      ...
      star(this.p, 0, 0, this.diameter * 0.2, this.radius, 12)
      this.p.fill(80)
      this.p.strokeWeight(3)
      this.p.stroke(this.app.red(0.7))
  ...
  pattern(): void {
    ...
    for (const enemy of this.party.enemies)
      if (!enemy.immune && this.calculatedTouch(enemy)) enemy.kill()
    for (const bonus of this.party.bonus)
      while (this.calculatedTouch(bonus)) bonus.placeOutOfViewport(true)
  }
