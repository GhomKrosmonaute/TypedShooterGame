...
export default class Freezer extends Enemy {
  ...
  constructor(party: Party) {
    ...
    this.overDraw = function () {
      if (this.freeze) this.freeze.drawImage(this.party.app.images.freeze)
    }
  }
  ...
  public pattern(): void {
    ...
    if (this.freeze && this.rawDist(this.freeze) > this.freezeRange)
      this.freeze = null
    if (this.freezeRate.canTrigger(true) && !this.freeze) this.launchFreeze()
    if (this.freeze) {
      this.freeze.follow(
        this.party.player,
      ...
      for (const enemy of this.party.enemies)
        if (
          ...
          !enemy.immune &&
          enemy.id !== "freezer" &&
          enemy.calculatedTouch(this.freeze)
        )
          this.exploseFreeze()
      if (this.freeze && this.party.player.calculatedTouch(this.freeze))
        this.exploseFreeze()
    }
    this.freezeExplosions = this.freezeExplosions.filter(
      (fe) => fe.time < this.party.time + this.freezeExplosionDuration
    )
    for (const fe of this.freezeExplosions) {
      for (const enemy of this.party.enemies)
        if (enemy.id !== "freezer") this.makeSlow(enemy)
    }
  }
