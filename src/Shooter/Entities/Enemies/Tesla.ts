...
export default class Tesla extends Enemy {
      ...
      for (const tesla of this.connections) {
        if (
          isOnArc(
            ...
            tesla,
            this.party.player,
            (tesla.arcWeight + this.arcWeight) * 0.5
          )
        ) {
  ...
  public pattern(): void {
      ...
      for (const tesla of this.connections)
        if (
          isOnArc(
            ...
            tesla,
            this.party.player,
            (this.arcWeight + tesla.arcWeight) * 0.5
          )
        ) {
    ...
    let target: Vector2D = [
      { x: this.arcSize * -0.4, y: 0 },
      { x: this.arcSize * 0.4, y: 0 },
      { x: 0, y: this.arcSize * -0.4 },
      { x: 0, y: this.arcSize * 0.4 },
      { x: this.arcSize * -0.4, y: this.arcSize * -0.4 },
      { x: this.arcSize * -0.4, y: this.arcSize * 0.4 },
      { x: this.arcSize * 0.4, y: this.arcSize * -0.4 },
      { x: this.arcSize * 0.4, y: this.arcSize * 0.4 },
    ][this.cardinalIndex]
  ...
  kill(addToScore: boolean = false) {
    ...
    this.party.enemies.forEach((enemy) => {
      if (enemy.id === "tesla")
        (enemy as Tesla).connections = (enemy as Tesla).connections.filter(
          (tesla) => tesla !== this
        )
    })
  ...
  arc(tesla: Tesla): void {
    arc(this.app, this, tesla, (this.arcWeight + tesla.arcWeight) * 0.5)
  }
