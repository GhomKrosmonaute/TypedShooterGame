...
export default class Angle {
  constructor(private p: p5, public degrees: number = 0) {}
  ...
  public pointTo(
    angle: Angle,
    speedDegrees: number = 360,
    ignoreOpposite: boolean = false
  ): Angle {
  ...
  static pathFinder(
    angle: Angle,
    target: Angle
  ): { dist: number; sens: number } {
    const dist = { toUp: 0, toDown: 0 }
    let newAngle = new Angle(angle.p, angle.degrees)
    for (let i = 0; i < 181; i++) {
      if (Math.floor(newAngle.degrees) === Math.floor(target.degrees)) break
      newAngle.move(-1)
      dist.toDown++
    }
  ...
  static between(p: p5, a: Vector2D, b: Vector2D): Angle {
    ...
    return new Angle(p, p.degrees(p.atan2(b.y - a.y, b.x - a.x) + p.PI))
  }
  ...
  static fromDirectionalKeys(app: App, type: "shoot" | "move"): Angle {
    if (
      ...
      return new Angle(app.p, 180)
    } else if (
      app.keyIsPressed(type, "up") &&
      app.keyIsPressed(type, "right")
    ) {
      return new Angle(app.p, 135)
    } else if (
      app.keyIsPressed(type, "down") &&
      app.keyIsPressed(type, "right")
    ) {
      return new Angle(app.p, 225)
    } else if (
      app.keyIsPressed(type, "down") &&
      app.keyIsPressed(type, "left")
    ) {
      return new Angle(app.p, 315)
    } else if (app.keyIsPressed(type, "up") && app.keyIsPressed(type, "left")) {
