...
export default class Form extends Zone {
  ...
  constructor(
    ...
    inputs.forEach((input, index) => {
      const position = this.fraction(
        0.5,
        (1 / inputs.length) * index + 1 / (inputs.length * 2)
      )
      const size = this.fraction(0.8, (1 / inputs.length) * 0.7, true)
      this.inputs.push(
        new Input(app, position.x, position.y, size.x, size.y, true, input)
      )
    })
  }
  ...
  keyPressed(key: string) {
    if (this.focus) {
      if (key === "Enter") {
        if (
          this.inputs.every(
            (input) => !input.required || input.value.length > 0
          )
        )
          this.sent(this)
      } else if (key === "Tab") {
        ...
        for (let i = 0; i < this.inputs.length; i++)
          if (this.inputs[i] === focused) {
            if (i + 1 >= this.inputs.length) this.inputs[0].focus = true
  ...
  mousePressed() {
    for (const input of this.inputs) {
      if (input.touchVector(this.app.mouseFromCenter)) input.focus = true
