...
export default class Link extends Positionable {
    ...
    draw(): void {
        this.x = this.p.width * this.fractionX - this.p.width * .5
        this.y = this.p.height * this.fractionY - this.p.height * .5
        this.p.fill(this.scene.app.white,this.rawTouch(this.scene.app.mouseFromCenter) ? 255 : 100)
        this.p.noStroke()
        this.p.textSize(30)
