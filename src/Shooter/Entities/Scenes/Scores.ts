...
export default class Scores extends Scene {
  ...
  async reset() {
    ...
    this.leaderBoard = await this.app.api.get<LeaderBoard>("leaderboard", {
      mode: this.mode,
    })
  }
  ...
  drawLeaderBoard(): void {
    ...
    this.leaderBoard.forEach((player, index) => {
      ...
      this.p.textSize(
        constrain(
          map(
            dist(0, this.app.mouseFromCenter.y, 0, rankZone.center.y),
            0,
            30,
            rankZone.fractionHeight(0.9),
            rankZone.fractionHeight(0.7)
          ),
          rankZone.fractionHeight(0.7),
          rankZone.fractionHeight(0.9)
        )
      )
      const rank = rankZone.fraction(1 / 12, 0.5)
      const name = rankZone.fraction(3 / 12, 0.5)
      const score = rankZone.fraction(5 / 12, 0.5)
      const prec = rankZone.fraction(7 / 12, 0.5)
      const kills = rankZone.fraction(9 / 12, 0.5)
      const duration = rankZone.fraction(11 / 12, 0.5)
      ...
      this.p.text(
        `prec: ${Math.round(player.precision * 100)}%`,
        prec.x,
        prec.y
      )
