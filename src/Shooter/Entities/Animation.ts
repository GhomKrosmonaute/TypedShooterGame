...
export default class Animation {
  ...
  constructor(public scene: Scene, options: AnimationOptions) {
    this.p = scene.p
    ...
    if (options.id) {
      this.id = options.id
      scene.animations[this.className] = scene.animations[
        this.className
      ].filter((a) => {
  ...
  public draw(): void {
    if (!isOutOfViewPort(this.position)) this.onDraw(this)
  }
  ...
  public map(...steps: number[]): number {
    ...
    const currentStep = Math.floor(
      map(this.time, 0, this.duration, 0, steps.length - 1.0001)
    )
    return map(
      this.time,
      0,
