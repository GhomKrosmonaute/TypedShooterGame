...
class Particle extends Positionable {
  ...
  private reset() {
    ...
    this.x = this.p.random(this.p.width * -0.5, this.p.width * 0.5)
    this.y = this.p.random(this.p.height * -0.5, this.p.height * 0.5)
  }

  public move(x: number, y: number) {
    super.move(x * (this._diameter / 2), y * (this._diameter / 2))
  }

  public step() {
    if (this.intensity < 1) this.intensity += 0.023
    if (!this.isOnScreen()) this.reset()
  }
  ...
  public draw() {
    ...
    this.p.ellipse(this.x, this.y, 5 + this._diameter * 5)
  }
}
