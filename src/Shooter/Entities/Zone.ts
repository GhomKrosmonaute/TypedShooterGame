...
export default class Zone {
  ...
  constructor(
    ...
    this.startX = toCenter ? x1 - x2 * 0.5 : x1
    this.startY = toCenter ? y1 - y2 * 0.5 : y1
    this.stopX = toCenter ? x1 + x2 * 0.5 : x2
    this.stopY = toCenter ? y1 + y2 * 0.5 : y2
  }

  public get start(): Vector2D {
    return { x: this.startX, y: this.startY }
  }
  public get stop(): Vector2D {
    return { x: this.stopX, y: this.stopY }
  }
  public get leftBottom(): Vector2D {
    return { x: this.startX, y: this.stopY }
  }
  public get rightUp(): Vector2D {
    return { x: this.stopX, y: this.startY }
  }
  public get width(): number {
    return this.stopX - this.startX
  }
  public get height(): number {
    return this.stopY - this.startY
  }
  public get center(): Vector2D {
    return this.fraction(0.5, 0.5)
  }
  ...
  public debug(p: p5): void {
    ...
    p.stroke(255, 0, 0)
    p.strokeWeight(1)
    p.rect(this.start.x, this.start.y, this.width, this.height)
  }
}
