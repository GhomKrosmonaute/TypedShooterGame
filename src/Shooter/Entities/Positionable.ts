...
export default class Positionable {
  ...
  public get diameter(): number {
    return this.currentDiameter || this._diameter
  }
  ...
  public constrain(zone?: Zone): Vector2D {
    ...
    return {
      x: constrain(this.x, this.p.width * -0.5, this.p.width * 0.5),
      y: constrain(this.y, this.p.height * -0.5, this.p.height * 0.5),
    }
  }
  ...
  public isOnScreen(ignoreRadius: boolean = false): boolean {
    ...
    return (
      this.x + radius > this.p.width * -0.5 &&
      this.x - radius < this.p.width * 0.5 &&
      this.y + radius > this.p.height * -0.5 &&
      this.y - radius < this.p.height * 0.5
    )
  }
