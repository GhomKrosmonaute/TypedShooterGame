...
export default class App {
  ...
  public async step() {
    if (this.rate.canTrigger(true)) {
      this.cursor.step()
      if (this.lightMode) {
        if (this.lightModeTransition > 0) this.lightModeTransition -= 25.5
      } else {
        if (this.lightModeTransition < 255) this.lightModeTransition += 25.5
      }
    }
    if (this.scene.rate.canTrigger(true)) {
      for (const className in this.scene.animations)
        if (this.scene.animations[className]) {
          this.scene.animations[className] = this.scene.animations[
            className
          ].filter((a) => !a.finish)
    ...
    if (this.hardcore) this.hardcoreVariator.step()
  }

  public async draw() {
    this.p.background(this.black)
    this.p.translate(this.p.width * 0.5, this.p.height * 0.5)
    if (this.scene.showParticles) this.particles.draw()
    this.scene.draw()
    ...
    this.p.textSize(15)
    this.p.text(
      `Shooter Game v${this.version} © Ghom`,
      0,
      this.p.height * 0.5 - 15
    )
    if (this.hardcore) {
      ...
      this.p.text(
        "HARDCORE MODE",
        0,
        this.p.height * 0.5 - (40 + this.hardcoreVariator.value)
      )
    }
    this.cursor.draw()
  }
  ...
  public get zone(): Zone {
    return new Zone(0, 0, this.p.width, this.p.height, true)
  }
  ...
  public switchKeyMode(): void {
    this.keyModeIndex++
    if (this.keyModeIndex >= this.keyModes.length) this.keyModeIndex = 0
    this.keys = {}
  ...
  public get white(): number {
    return 255 - this.lightModeTransition
  }
  ...
  public alpha(proportion: number = 1): p5.Color {
    return this.red(0.5, proportion)
  }
  ...
  public keyIsPressed(
    ...
    for (const key in this.keys)
      if (this.keys[key])
        if (this.keyMode[type][direction].includes(key)) return true
    return false
  }
  ...
  public keyPressed(key: string): void {
    ...
    if (!this.scene.form || !this.scene.form.focus) {
      ...
      else if (this.keyMode.shortcuts.keyMode.includes(key))
        this.switchKeyMode()
      else if (this.keyMode.shortcuts.gamepad.includes(key))
        this.switchGamepad()
      else
  ...
  public moveKeyIsPressed(): boolean {
    return this.directionalKeyIsPressed("move")
  }

  public shootKeyIsPressed(): boolean {
    return this.directionalKeyIsPressed("shoot")
  }
  ...
  public touchStarted(event: any): void {
    if (this.mouseFromCenter.y > this.p.height * 0.3) {
      // TODO: use activable
    } else {
  ...
  public get touchAngle(): Angle | false {
    ...
    return Angle.between(this.p, this.touch.base, this.touch.current)
  }
  ...
  public switchGamepad(): void {
    ...
    if (this.useGamepad)
      this.scene.setPopup(`Using "${this.gamepad.id}" gamepad`)
