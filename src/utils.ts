...
import RotationSpeedUp from "./Shooter/Entities/Bonus/RotationSpeedUp"
  ...
  return constrain(
    Math.min(
      map(
        ...
        0,
        fadeOut.valueMax,
        fadeOut.fadeMax || fadeMax * fadeOut.overflow,
        0
      ),
      map(
        fadeIn.value,
        ...
        fadeIn.valueMax,
        0,
        fadeIn.fadeMax || fadeMax * fadeIn.overflow
      )
    ),
    0,
    fadeMax
...
export function pickEnemy(party: Party): Enemy {
  ...
  switch (rdm) {
    case 0:
      return new ShieldPiercer(party)
    case 1:
      return new BlobMob(party)
    case 2:
      return new CircularSaw(party)
    case 3:
      return new Tesla(party)
    case 4:
      return new Rocket(party)
    case 5:
      return new Pulsar(party)
    case 6:
      return new Freezer(party)
  }
}

export function pickBonus(party: Party): Bonus {
  const rdm = Math.floor(Math.random() * 15)
  switch (rdm) {
    case 0:
      return new Heal(party)
    case 1:
      return new StarBalls(party)
    case 2:
      return new DeadlyWave(party)
    case 3:
      return new PiercingShots(party)
    case 4:
      return new AutoFireGuidance(party)
    case 5:
      return new Shield(party)
    case 6:
      return new DamageUp(party)
    case 7:
      return new ShotsSizeUp(party)
    case 8:
      return new FireRateUp(party)
    case 9:
      return new RangeUp(party)
    case 10:
      return new DeadChain(party)
    case 11:
      return new SpeedUp(party)
    case 12:
      return new ExplosiveShots(party)
    case 13:
      return new ShotsSpeedUp(party)
    case 14:
      return new RotationSpeedUp(party)
  }
}
...
export function isOnScreen(p: p5, vector: Vector2D): boolean {
  return (
    vector.x > p.width * -0.5 &&
    vector.x < p.width * 0.5 &&
    vector.y > p.height * -0.5 &&
    vector.y < p.height * 0.5
  )
}
...
export function rawDist(vector1: Vector2D, vector2: Vector2D): number {
  return dist(vector1.x, vector1.y, vector2.x, vector2.y)
}
...
export function fromCenter(p: p5, vector: Vector2D): Vector2D {
  return {
    x: vector.x - p.width * 0.5,
    y: vector.y - p.height * 0.5,
  }
}
